...
private:
    ...
    std::atomic_flag _callback_flag = ATOMIC_FLAG_INIT;
...
public:
    ...
    virtual int open(const std::string& device, int w, int h, int type) override
    {
        ...
        if (ioctl(_v4l2_fd, VIDIOC_G_PARM, &_parm) < 0) {
            perror("获取帧率失败");
            ret = -4;
        }
    ...
    virtual int get_vi(void** data) override
    {
        ...
        int ret = select(_v4l2_fd + 1, &fds, NULL, NULL, &tv);
        if (ret == -1) {
            perror("select error");
            return -3;
        } else if (ret == 0) {
    ...
    virtual int connect(d_hal_vin_callback_t func) override
    {
        ...
        if (_thread == nullptr) {
            _thread = std::make_shared<std::thread>(&d_hal_v4l2::async_get_vi_thread, this);
        }
        return 0;
    }
