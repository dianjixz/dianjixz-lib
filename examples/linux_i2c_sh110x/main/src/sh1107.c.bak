...
void sh1107_display_text(SH1107_t * dev, int row, int col, char * text, int text_len, bool invert)
{
	...
	printf("_direction=%d _width=%d _height=%d _length=%d _row=%d _col=%d", 
	...
	if (scroll == SCROLL_RIGHT) {
		...
		for (int page=0;page<dev->_pages-1;page++) {
			...
			for (int seg=_start;seg<=_end;seg++) {
				...
				if (seg == 0) {
					printf("b page=%d wk0=%02x wk1=%02x", page, wk0, wk1);
				}
				wk0 = wk0 >> 1;
				wk1 = wk1 & 0x01;
				wk1 = wk1 << 7;
				wk2 = wk0 | wk1;
				if (seg == 0) {
					printf("a page=%d wk0=%02x wk1=%02x wk2=%02x", page, wk0, wk1, wk2);
				}
...
{
	if ( (width % 8) != 0) {
		printf("width must be a multiple of 8");
		return;
	}
	if ( (xpos % 8) != 0) {
		printf("xpos must be a multiple of 8");
		return;
	}
	int _width = width / 8;
	//uint8_t wk0;
...
{
	printf("src=%02x srcBits=%d dst=%02x dstBits=%d", src, srcBits, dst, dstBits);
...
{
	printf("dev->_page[%d]._segs[%d]=%02x", page, seg, dev->_page[page]._segs[seg]);
}