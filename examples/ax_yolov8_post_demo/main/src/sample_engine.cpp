                           ...
                           const std::vector<std::vector<uint8_t> > &skeleton, const char *output_name)
{
    ...
    for (size_t i = 0; i < objects.size(); i++) {
        ...
        cv::rectangle(bgr, obj.rect, cv::Scalar(255, 0, 0, 255), 5);
        ...
        cv::putText(bgr, text, cv::Point(x, y + label_size.height), cv::FONT_HERSHEY_SIMPLEX, 0.5,
                    cv::Scalar(0, 0, 0, 255));
        ...
        for (int j = 0; j < num_point + 2; j++) {
            ...
            if (j < num_point) {
                ...
                if (kps_s > 0.5f) {
                    ...
                    cv::circle(bgr, {kps_x, kps_y}, 5, kps_color, -1);
                }
            }
            // draw line
            auto &ske  = skeleton[j];
            ...
            if (pos1_s > 0.5f && pos2_s > 0.5f) {
                ...
                cv::line(bgr, {pos1_x, pos1_y}, {pos2_x, pos2_y}, limb_color, 5);
            }
        }
    }
}
