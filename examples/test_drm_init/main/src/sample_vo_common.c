...
static AX_VOID *SAMPLE_VO_CHN_THREAD(AX_VOID *pData)
{
    ...
    while (!pstChnThreadParam->u32ThreadForceStop) {
        ...
        {
            SAMPLE_PRT("layer%d-chn%d AX_POOL_GetBlock success\n", u32LayerID, u32ChnID);
        }
        ...
        if ((stFrame.stCompressInfo.enCompressMode == AX_COMPRESS_MODE_NONE)
            && (strcmp(pstChnThreadParam->chnFileName,"") == 0)) {
                // memset(u64VirAddr, 0x0f, stFrame.u32Width * stFrame.u32Height);
            ...
            // printf(
            //     "SAMPLE_Fill_Color(stFrame.enImgFormat:%d,stFrame.u32Width:%d,stFrame.u32Height:%d,stFrame.u32PicStride[0],u64VirAddr);"


            // "\n"
            // ,
            // stFrame.enImgFormat, stFrame.u32Width, stFrame.u32Height
            // );
