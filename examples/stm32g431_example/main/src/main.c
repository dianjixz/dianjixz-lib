...
int main(void)
{
  ...
  __HAL_TIM_SetCompare(&htim1, TIM_CHANNEL_1, 0);     // 风扇转速0%
  ...
  oled_res = ssd1306_basic_init(SSD1306_INTERFACE_IIC, SSD1306_ADDR_SA0_0);
  if (oled_res != 0)
  {
      // return 1;
  }
  ...
  {
    oled_res = ssd1306_basic_clear();
    if (oled_res == 0)
    {
      oled_res = ssd1306_basic_string(0, 0, "AI pyramid", 16, 1, SSD1306_FONT_24);
    }
  }
