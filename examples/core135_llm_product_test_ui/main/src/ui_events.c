...
// 		lv_obj_set_style_bg_color(ui_Button8, lv_color_hex(0xF8BE4E), LV_PART_MAIN | LV_STATE_DEFAULT);
...
// 		lv_obj_set_style_bg_color(ui_Button3, lv_color_hex(0xF8BE4E), LV_PART_MAIN | LV_STATE_DEFAULT);
...
const void *map_index_obj[6] = {&ui_Panel8, &ui_Panel10, &ui_Panel11, &ui_Panel12, &ui_Panel13, &ui_Panel14};
void start_hw_info();
void stop_hw_info();
void ui_switch_led(lv_event_t *e) {
    int len = lv_tabview_get_tab_act(ui_TabView3);
...
void lvgl_task_get_system_cb(lv_timer_t *tmr) {
    ...
    if (laster.time != 0) {
        ...
        lv_label_set_text(ui_Label20, cpu_str);
        ...
        lv_label_set_text(ui_Label23, mem_str);
    } else {
        memcpy(&laster, &now, sizeof(struct cpu_use_t));
    }
    lv_label_set_text(ui_Label24, get_times());

    {
        static int old_status = 0;
        int status            = ax_llm_connect_flage;
        if (status == 0) {
            lv_color_t color     = lv_obj_get_style_bg_color(ui_Image4, LV_PART_MAIN);
