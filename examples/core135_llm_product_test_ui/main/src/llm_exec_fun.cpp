...
std::string sample_json_str_get(const std::string &json_str, const std::string &json_key) {
    std::string key_val;
    // std::cout << "json_str: " << json_str << " json_key:" << json_key << std::endl;
...
class llm_uart_exec {
   ...
   public:
    llm_uart_exec(const std::string &dev_name = "/dev/ttySTM2") {
        exit_flage   = 0;
        alloc_commit = 1;
...
void ax_cmmc_test_task() {
    ...
    int summ[2]        = {0};
    ui_lock();
    lv_textarea_add_text(ui_TextArea2, "test llm emmc.\n");
    // lv_chart_set_range(ui_Chart1, LV_CHART_AXIS_SECONDARY_Y, 0, 200);
    ui_Chart1_series_1       = lv_chart_get_series_next(ui_Chart1, NULL);
    ui_Chart1_series_2       = lv_chart_get_series_next(ui_Chart1, ui_Chart1_series_1);
    ui_Chart1_series_1_array = lv_chart_get_y_array(ui_Chart1, ui_Chart1_series_1);
    ui_Chart1_series_2_array = lv_chart_get_y_array(ui_Chart1, ui_Chart1_series_2);
    ...
    int success_count = 0;
    {
                ...
                if (true_s != std::string::npos) {
                    ...
                    str_parse_line(delta, lines, flage, [&](const std::string &_lin) {
                        if (_lin.find("Creating") != std::string::npos) {
                            ui_lock();
                            lv_textarea_add_text(ui_TextArea2, ".");
                            ui_unlock();
                        }
                        if (_lin.find("latency") != std::string::npos) {
                            ...
                            if (reads > 0) {
                                ...
                                std::string lab_tmp = "AR:" + std::to_string((int)Ravg_s) + "M/s";
                                lv_label_set_text(ui_Label3, lab_tmp.c_str());
                                lv_chart_set_ext_y_array(ui_Chart1, ui_Chart1_series_1, ui_Chart1_series_1_array);
                                {
                                    ...
                                    max_value += 25;
                                    lv_chart_set_range(ui_Chart1, LV_CHART_AXIS_SECONDARY_Y, min_value, max_value);
                                    // lv_chart_refresh(ui_Chart1);
                                }
                            }
                            if (writes > 0) {
                                ...
                                std::string lab_tmp = "AW:" + std::to_string((int)Wavg_s) + "M/s";
                                lv_label_set_text(ui_Label5, lab_tmp.c_str());
                                lv_chart_set_ext_y_array(ui_Chart1, ui_Chart1_series_2, ui_Chart1_series_2_array);
                                {
                                    lv_coord_t min_value = 1000;
                                    lv_coord_t max_value = 0;
                            ...
                            if (lv_obj_has_flag(ui_Chart1, LV_OBJ_FLAG_HIDDEN))
                                lv_obj_clear_flag(ui_Chart1, LV_OBJ_FLAG_HIDDEN);
                            if (!lv_obj_has_flag(ui_TextArea2, LV_OBJ_FLAG_HIDDEN))
        ...
        while (1) {
            ...
            if (WaitTime > 3) {
                ui_lock();
                lv_textarea_add_text(ui_TextArea2, "emmc test timeout!\n");
...
cmmc_task_e:
    ...
    if (error) {
        ui_lock();
        lv_textarea_add_text(ui_TextArea2, "emmc test false!\n");
        lv_obj_set_style_bg_color(ui_Button1, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT);
        ui_unlock();
    } else {
        ui_lock();
        lv_textarea_add_text(ui_TextArea2, "emmc test success!\n");
        lv_obj_set_style_bg_color(ui_Button1, lv_color_hex(0xFF00), LV_PART_MAIN | LV_STATE_DEFAULT);
        ui_unlock();
    }

    ui_lock();
    lv_obj_add_flag(ui_TabPage1, LV_OBJ_FLAG_SCROLL_CHAIN);
    lv_obj_clear_state(ui_Button1, LV_STATE_DISABLED);
    lv_obj_add_flag(ui_TextArea2, LV_OBJ_FLAG_CLICKABLE);
...
void ax_sd_test_task() {
    ...
    float Wavg_s = 0.0;
    ui_lock();
    lv_textarea_add_text(ui_TextArea8, "test llm sd.\n");
    ui_Chart8_series_1       = lv_chart_get_series_next(ui_Chart8, NULL);
    ui_Chart8_series_2       = lv_chart_get_series_next(ui_Chart8, ui_Chart8_series_1);
    ui_Chart8_series_1_array = lv_chart_get_y_array(ui_Chart8, ui_Chart8_series_1);
    ui_Chart8_series_2_array = lv_chart_get_y_array(ui_Chart8, ui_Chart8_series_2);
    ...
    ui_unlock();
    {
        ...
        uart_exec.bashexec(
            ...
            [&](const std::string &json_body) {
                ...
                if (json_body.find("mmcblk1p2_not_exists") != std::string::npos) {
                    ui_lock();
                    lv_textarea_add_text(ui_TextArea8, "mmcblk1p2 not exists!\n");
                ...
                if (true_s != std::string::npos) {
                    ...
                    str_parse_line(delta, lines, flage, [&](const std::string &_lin) {
                        if (_lin.find("Creating") != std::string::npos) {
                            ui_lock();
                            lv_textarea_add_text(ui_TextArea8, ".");
                            ui_unlock();
                        }
                        if (_lin.find("latency") != std::string::npos) {
                            int find_index;
                            ...
                            if (reads > 0) {
                                ...
                                std::string lab_tmp = "AR:" + std::to_string((int)Ravg_s) + "M/s";
                                lv_label_set_text(ui_Label50, lab_tmp.c_str());
                                lv_chart_set_ext_y_array(ui_Chart8, ui_Chart8_series_1, ui_Chart8_series_1_array);
                                {
                                    lv_coord_t min_value = 1000;
                                    lv_coord_t max_value = 0;
                                    ...
                                    max_value += 5;
                                    lv_chart_set_range(ui_Chart8, LV_CHART_AXIS_SECONDARY_Y, min_value, max_value);
                            ...
                            if (writes > 0) {
                                ...
                                std::string lab_tmp = "AW:" + std::to_string((int)Wavg_s) + "M/s";
                                lv_label_set_text(ui_Label51, lab_tmp.c_str());
                                lv_chart_set_ext_y_array(ui_Chart8, ui_Chart8_series_2, ui_Chart8_series_2_array);
                                {
                                    lv_coord_t min_value = 1000;
                                    lv_coord_t max_value = 0;
                                    ...
                                    min_value = min_value > 20 ? min_value - 20 : 0;
                                    max_value += 5;
                                    lv_chart_set_range(ui_Chart8, LV_CHART_AXIS_PRIMARY_Y, min_value, max_value);
                                    // lv_chart_refresh(ui_Chart1);
                                }
                            }
                            if (lv_obj_has_flag(ui_Chart8, LV_OBJ_FLAG_HIDDEN))
                                lv_obj_clear_flag(ui_Chart8, LV_OBJ_FLAG_HIDDEN);
                            if (!lv_obj_has_flag(ui_TextArea8, LV_OBJ_FLAG_HIDDEN))
                                lv_obj_add_flag(ui_TextArea8, LV_OBJ_FLAG_HIDDEN);
                            ui_unlock();
                        }
                    });
                }
        ...
        while (1) {
            ...
            if (WaitTime > 3) {
                ui_lock();
                lv_textarea_add_text(ui_TextArea8, "sd test timeout!\n");
                lv_obj_set_style_bg_color(ui_Button8, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT);
                ui_unlock();
                printf("dev disconnect!\n");
                uart_exec.clear_request_id(request_id_num);
...
sd_task_e:
    ...
    if (error) {
        ui_lock();
        lv_textarea_add_text(ui_TextArea8, "sd test false!\n");
        lv_obj_set_style_bg_color(ui_Button8, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT);
        ui_unlock();
    } else {
        ui_lock();
        lv_textarea_add_text(ui_TextArea8, "sd test success!\n");
        lv_obj_set_style_bg_color(ui_Button8, lv_color_hex(0xFF00), LV_PART_MAIN | LV_STATE_DEFAULT);
        ui_unlock();
    }
    ui_lock();
    lv_obj_add_flag(ui_TabPage2, LV_OBJ_FLAG_SCROLL_CHAIN);
    lv_obj_clear_state(ui_Button8, LV_STATE_DISABLED);
    lv_obj_add_flag(ui_TextArea8, LV_OBJ_FLAG_CLICKABLE);
...
void ax_cmm_test_task() {
    ...
    std::string lines;
    ui_lock();
    lv_textarea_add_text(ui_TextArea4, "test llm cmm.\n");
    ui_unlock();
    int success_count = 0;
    {
        int request_id_num = -1;
        ...
        uart_exec.bashexec("/usr/local/m5stack/bin/sample_cmm ", [&](const std::string &json_body) {
            ...
            if (true_s != std::string::npos) {
                ...
                std::string delta = sample_json_str_get(json_body, "delta");
                static int flage  = 1;
                str_parse_line(delta, lines, flage, [&success_count](const std::string &_lin) {
                    if (_lin.find("ax_mem_cmm_test_") != std::string::npos) {
                        ...
                        ui_lock();
                        lv_textarea_add_text(ui_TextArea4, _lin.c_str());
                        ui_unlock();
                    }
                });
            }
        ...
        while (1) {
            ...
            if (WaitTime > 3) {
                ui_lock();
                lv_textarea_add_text(ui_TextArea4, "cmm WaitTime timeout!\n");
                lv_obj_set_style_bg_color(ui_Button3, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT);
                ui_unlock();
                printf("dev disconnect!\n");
                uart_exec.clear_request_id(request_id_num);
...
cmm_task_e:
    ui_lock();
    if (success_count < 18) {
        lv_textarea_add_text(ui_TextArea4, "cmm test false!\n");
        lv_obj_set_style_bg_color(ui_Button3, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT);
    } else {
        lv_textarea_add_text(ui_TextArea4, "cmm test success!\n");
        lv_obj_set_style_bg_color(ui_Button3, lv_color_hex(0xFF00), LV_PART_MAIN | LV_STATE_DEFAULT);
    }

    lv_textarea_add_text(ui_TextArea4, "ax_llm cmm over!\n");
    lv_obj_add_flag(ui_TabPage4, LV_OBJ_FLAG_SCROLL_CHAIN);
    lv_obj_clear_state(ui_Button3, LV_STATE_DISABLED);
    lv_obj_add_flag(ui_TextArea4, LV_OBJ_FLAG_CLICKABLE);
    ui_unlock();
...
void ax_play_test_task() {
    int ret;
    std::string lines;
    lv_textarea_add_text(ui_TextArea5, "ax_llm start play logo.wav!\n");
    {
        sem_t sem;
        sem_init(&sem, 0, 0);
...
play_task_e:
    ui_lock();
    lv_textarea_add_text(ui_TextArea5, "ax_llm play over!\n");
    lv_obj_add_flag(ui_TabPage5, LV_OBJ_FLAG_SCROLL_CHAIN);
    lv_obj_clear_state(ui_Button4, LV_STATE_DISABLED);
    lv_obj_clear_state(ui_Button6, LV_STATE_DISABLED);
    lv_obj_clear_state(ui_Button9, LV_STATE_DISABLED);
    lv_obj_add_flag(ui_TextArea5, LV_OBJ_FLAG_CLICKABLE);
...
void ax_playcap_test_task() {
    int ret;
    std::string lines;
    lv_textarea_add_text(ui_TextArea5, "ax_llm start play cap:\n");
    {
        sem_t sem;
        sem_init(&sem, 0, 0);
...
playcap_task_e:
    ui_lock();
    lv_textarea_add_text(ui_TextArea5, "ax_llm play cap over!\n");
    lv_obj_add_flag(ui_TabPage5, LV_OBJ_FLAG_SCROLL_CHAIN);
    lv_obj_clear_state(ui_Button4, LV_STATE_DISABLED);
    lv_obj_clear_state(ui_Button6, LV_STATE_DISABLED);
    lv_obj_clear_state(ui_Button9, LV_STATE_DISABLED);
...
void cap_time() {
    ...
    while (cap_flage) {
        ...
        lv_textarea_add_text(ui_TextArea5, ".");
        ui_unlock();
    }
}
...
load_task_e:
    ui_lock();
    lv_obj_add_flag(ui_TabPage8, LV_OBJ_FLAG_SCROLL_CHAIN);
    ui_unlock();
}
    ...
    while (!hw_info_exit_flage) {
        commit = uart_exec.sys_hwinfo([&sem](const std::string &json_body) {
            ...
            try {
                ...
                lv_label_set_text(ui_Label36, str_buf);
                sprintf(str_buf, "%i%%", (int)req_body["data"]["mem"]);
                lv_label_set_text(ui_Label39, str_buf);
                sprintf(str_buf, "%iC", (int)(((int)req_body["data"]["temperature"]) / 1000));
                lv_label_set_text(ui_Label40, str_buf);
                ...
                if (req_body["data"].contains("eth_info")) {
                    for (const auto& device : req_body["data"]["eth_info"]) {
                        ...
                        if(name == "eth0")
                        {
                            ...
                            lv_label_set_text(ui_Label11111, str_buf);
                        }
                    }
                }
            } catch (...) {
...
void ax_cmmc_test(lv_event_t *e) {
    ...
    if (lv_obj_has_flag(ui_TextArea2, LV_OBJ_FLAG_HIDDEN)) lv_obj_clear_flag(ui_TextArea2, LV_OBJ_FLAG_HIDDEN);
    if (!lv_obj_has_flag(ui_Chart1, LV_OBJ_FLAG_HIDDEN)) lv_obj_add_flag(ui_Chart1, LV_OBJ_FLAG_HIDDEN);

    if (ax_llm_connect_flage == 0) {
        lv_textarea_add_text(ui_TextArea2, "ax_llm not't connect!\n");
        lv_obj_set_style_bg_color(ui_Button1, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT);
        return;
    }
    // page_lock = 1;
    ...
    lv_obj_clear_flag(ui_TextArea2, LV_OBJ_FLAG_CLICKABLE);
...
void ax_sd_test(lv_event_t *e) {
    ...
    if (lv_obj_has_flag(ui_TextArea8, LV_OBJ_FLAG_HIDDEN)) lv_obj_clear_flag(ui_TextArea8, LV_OBJ_FLAG_HIDDEN);
    if (!lv_obj_has_flag(ui_Chart8, LV_OBJ_FLAG_HIDDEN)) lv_obj_add_flag(ui_Chart8, LV_OBJ_FLAG_HIDDEN);

    if (ax_llm_connect_flage == 0) {
        lv_textarea_add_text(ui_TextArea8, "ax_llm not't connect!\n");
        lv_obj_set_style_bg_color(ui_Button8, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT);
        return;
    }
    // page_lock = 1;
    lv_obj_clear_flag(ui_TabPage2, LV_OBJ_FLAG_SCROLL_CHAIN);
    lv_obj_add_state(ui_Button8, LV_STATE_DISABLED);
    lv_obj_clear_flag(ui_TextArea8, LV_OBJ_FLAG_CLICKABLE);
...
void ax_eth_test(lv_event_t *e) {
    ...
    if (!lv_obj_has_flag(ui_Chart3, LV_OBJ_FLAG_HIDDEN)) lv_obj_add_flag(ui_Chart3, LV_OBJ_FLAG_HIDDEN);
    ...
    std::thread t(ax_eth_test_task);
    t.detach();
}

void ax_uart_test(lv_event_t *e) {
    //     if (ax_uart_test_task_flage) {
    //         lv_obj_set_style_bg_color(ui_Button5, lv_color_hex(0x79B4F2), LV_PART_MAIN | LV_STATE_DEFAULT);
    //         ax_uart_test_task_flage = 0;
    //     } else {
    //         lv_obj_set_style_bg_color(ui_Button5, lv_color_hex(0xF8BE4E), LV_PART_MAIN | LV_STATE_DEFAULT);
    //         lv_textarea_set_text(ui_TextArea3, "");
    //         // lv_textarea_set_text(ui_TextArea9, "");
    //         if (ax_llm_connect_flage == 0) {
    //             lv_textarea_add_text(ui_TextArea3, "ax_llm not't connect!\n");
    //             lv_obj_set_style_bg_color(ui_Button5, lv_color_hex(0xFF0000), LV_PART_MAIN | LV_STATE_DEFAULT);
    //             return;
    //         }

    //         lv_obj_clear_flag(ui_TabPage6, LV_OBJ_FLAG_SCROLL_CHAIN);
    //         // lv_obj_add_state(ui_Button5, LV_STATE_DISABLED);
    //         lv_obj_clear_flag(ui_TextArea3, LV_OBJ_FLAG_CLICKABLE);
    //         // lv_obj_clear_flag(ui_TextArea9, LV_OBJ_FLAG_CLICKABLE);
    //         ax_uart_test_task_flage = 1;
    //         std::thread t(ax_uart_test_task);
    //         t.detach();
    //     }
}
...
void ax_cap_test(lv_event_t *e) {
    if (cap_flage) {
        ...
        cap_flage = 1;
        lv_obj_set_style_bg_color(ui_Button4, lv_color_hex(0xF8BE4E), LV_PART_MAIN | LV_STATE_DEFAULT);
        ...
        // page_lock = 1;
        lv_obj_clear_flag(ui_TabPage5, LV_OBJ_FLAG_SCROLL_CHAIN);
