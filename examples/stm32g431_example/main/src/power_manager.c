...
void system_power_boot(void)
{
    ...
    // HAL_GPIO_WritePin(DL_MODE_GPIO_Port, DL_MODE_Pin, GPIO_PIN_SET);  // 上电的时候默认是低，此时拉高
    // HAL_Delay(100);

    HAL_GPIO_WritePin(SYS_RST_GPIO_Port, SYS_RST_Pin, GPIO_PIN_SET);  // 上电的时候默认是低，此时拉高

    // HAL_Delay(500);
    // HAL_GPIO_WritePin(DL_MODE_GPIO_Port, DL_MODE_Pin, GPIO_PIN_RESET);

    // 打开风扇PWM
    __HAL_TIM_SetCompare(&htim1, TIM_CHANNEL_1, 3400*2);  // 风扇转速100%
