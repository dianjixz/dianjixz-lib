...
int gpioWrite(int pin, int value)
{
    ...
    #endif
    char command[128];
    // sprintf(command, "gpioset gpiochip%d %d=%d", pin / 32, pin % 32, value);
    #if USE_USB
    status &= ~TIOCM_DTR;
    status &= ~TIOCM_RTS;
    #endif
    ...
    {
        // TARGET_RST_Pin 引脚
        sprintf(command, "ax650_ec_cli device --wifi_switch -d %d", value);
       #if USE_USB
        status |= TIOCM_RTS;
        #endif
    ...
    else
    {
        ...
        status |= TIOCM_DTR;
        #endif
        sprintf(command, "gpioset gpiochip2 0=%d", value);
    }
