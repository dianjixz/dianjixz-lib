...
//         // 判断是否相等
//         uint8x16_t vcmp = veorq_u8(v1, v2); // 异或后全0表示相等
//         if (!vminvq_u8(vcmp) == 0) { // 只要有一位不等
//             break;
//         }
//         p1 += 16;
//         p2 += 16;
...
// }
/*
* test allocate memory, map cache fast, invalidate cache
*/
static int ax_mem_cmm_test_025(void)
{
    ...
    for(i = 0; i < check_size; i++) {
        pVirAddr2[i] = AX_SYS_MmapCacheFast(PhyAddr[i], BlockSize);
        if(pVirAddr2[i] == NULL){
            printf("AX_SYS_MmapCacheFast failed,i=%d\n", i);
            return -1;
        }else{
            // printf("PhyAddr= 0x%llx,new cached pVirAddr2=%p\n",PhyAddr[i],pVirAddr2[i]);
        }
    }
    for (i = 1; i < check_size; i++) {
        memset((AX_U8 *)pVirAddr2[i], 0xbc, BlockSize);
    }
    // memset((AX_U8 *)pVirAddr2[16], 0xaa, BlockSize);
    for (i = 1; i < check_size; i++) {
        nRet = AX_SYS_MinvalidateCache(PhyAddr[i], pVirAddr2[i], BlockSize);
        if (nRet != 0) {
            printf("AX_SYS_MinvalidateCache failed\n");
            return -1;
        }
    ...
    for (i = 1; i < check_size; i++) {
        if(memcmp((AX_U8 *)pVirAddr2[0], (AX_U8 *)pVirAddr2[i], BlockSize) != 0){
            printf("ax_mem_cmm_test_025 failed,i=%d\n", i);
            return -1;
        }
        if(i%128 == 0){
    ...
    for(i = 1; i < check_size; i++) {
        nRet = AX_SYS_Munmap(pVirAddr2[i], BlockSize);
        if (nRet != 0) {
            printf("AX_SYS_Munmap failed\n");
            return -1;
        }
