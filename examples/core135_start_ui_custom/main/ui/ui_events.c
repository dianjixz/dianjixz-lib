...
/********************************GPS******************************************* */
static int communication_gps_count = 0;
pthread_t test_gps_head;

void gps_set_ui(int count, char *token)
{
    char lable_buff[64];
    switch (count) {
        case 0:  // 消息ID
            break;
        case 1:  // UTC时间
            lv_label_set_text(ui_Label[25], token);
            break;
        case 2:  // 纬度
            if (token[0] != '\0')
                lv_label_set_text(ui_Label[27], token);
            else
                lv_label_set_text(ui_Label[27], " ");
            break;
        case 3:  // 纬度半球
            if (token[0] != '\0') {
                sprintf(lable_buff, "%s:", token);
                lv_label_set_text(ui_Label[23], lable_buff);
            } else {
                lv_label_set_text(ui_Label[23], " ");
            }
            break;
        case 4:  // 经度
            if (token[0] != '\0') {
                lv_label_set_text(ui_Label[28], token);
            } else
                lv_label_set_text(ui_Label[28], " ");
            break;
        case 5:  // 经度半球
            if (token[0] != '\0') {
                sprintf(lable_buff, "%s:", token);
                lv_label_set_text(ui_Label[24], lable_buff);
            } else
                lv_label_set_text(ui_Label[24], " ");
            break;
        case 6:  // GPS状态
            break;
        case 7:  // 在视卫星数
            if (token[0] != '\0') {
                lv_label_set_text(ui_Label[26], token);
                if (strcmp(token, "00") == 0)
                    lv_obj_set_style_bg_color(ui_Container[12], lv_color_hex(0xFF0000),
                                              LV_PART_MAIN | LV_STATE_DEFAULT);
                else
                    lv_obj_set_style_bg_color(ui_Container[12], lv_color_hex(0xFF00), LV_PART_MAIN | LV_STATE_DEFAULT);
            } else
                lv_label_set_text(ui_Label[26], " ");
            break;
        default:
            break;
    }
...
static void *test_gps_pthread(void *arg)
{
    system("echo 1 > /sys/kernel/config/device-tree/overlays/uart2/status ; sleep 0.1");
    uart_t par        = {.baud = 115200, .data_bits = 8, .stop_bits = 1, .parity = 'n', .wait_flage = 1};
    char buffer[1024] = {0};
    char *buffer_p    = &buffer[0];
    pthread_mutex_lock(&device_thread_mutex);
    lv_textarea_set_text(ui_TextArea[12], "");
    lv_textarea_add_text(ui_TextArea[12], "open\n");
    pthread_mutex_unlock(&device_thread_mutex);
    int _serial = linux_uart_init("/dev/ttySTM2", &par);
    if (_serial <= 0) {
        pthread_mutex_lock(&device_thread_mutex);
        lv_textarea_add_text(ui_TextArea[12], "faile\n");
        pthread_mutex_unlock(&device_thread_mutex);
        return NULL;
    }
    int time_count = 0;
    ...
    while (communication_gps_count) {
        usleep(5 * 1000);
        for (int len = linux_uart_read(_serial, 1, buffer_p++); (len > 0) && (buffer_p[-1] != '\n');
    ...
    linux_uart_deinit(_serial);
    pthread_mutex_lock(&device_thread_mutex);
    lv_textarea_add_text(ui_TextArea[12], "close\n");
    pthread_mutex_unlock(&device_thread_mutex);
    system("echo 0 > /sys/kernel/config/device-tree/overlays/uart2/status");
    return NULL;
}
...
void screen_test_gps_event(int e, bool state)
{
    ...
    old_state = state;
    if (old_state) {
        communication_gps_count = 1;
        pthread_create(&test_gps_head, NULL, test_gps_pthread, NULL);
    } else {
        communication_gps_count = 0;
    }
}