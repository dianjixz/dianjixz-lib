...
//code struct video_timing video_1920x1080_120Hz   =   {88,  44, 148, 1920,  2200,  4,  5,  36, 1080, 1125, 1,1,0, AR_16_9,297000};
//code struct video_timing video_1920x1080_50Hz   =    {528, 44, 148, 1920,  2640,  4,  5,  36, 1080, 1125, 1,1,31,AR_16_9,148500};
//code struct video_timing video_1920x1080_30Hz   =    {88,  44, 148, 1920,  2200,  4,  5,  36, 1080, 1125, 1,1,34,AR_16_9,74250};
//code struct video_timing video_1920x1080_25Hz   =    {528, 44, 148, 1920,  2640,  4,  5,  36, 1080, 1125, 1,1,33,AR_16_9,74250};
//code struct video_timing video_1920x1080_24Hz   =    {638, 44, 148, 1920,  2750,  4,  5,  36, 1080, 1125, 1,1,32,AR_16_9,74250};

code struct video_timing video_3840x2160_30Hz   =    {176,  88, 296, 3840, 4400,  8,  10, 72, 2160, 2250, 1,1,95,AR_16_9, 297000};
//code struct video_timing video_3840x2160_25Hz   =    {1056, 88, 296, 3840, 5280,  8,  10, 72, 2160, 2250, 1,1,94,AR_16_9, 297000};
//code struct video_timing video_3840x2160_24Hz   =    {1276, 88, 296, 3840, 5500,  8,  10, 72, 2160, 2250, 1,1,93,AR_16_9, 297000};

//VESA-DMT
//code struct video_timing video_1440x2560_70Hz   =    {50, 20,  50,  1440,  1560,  9,  3,  13, 2560,  2585,   1,1,0,AR_16_9,282300};
code struct video_timing video_1366x768_60Hz    =    {70, 143, 213, 1366,  1792,  3,  3,  24, 768,   798,    1,1,0,AR_16_9,85500};
//code struct video_timing video_1400x1050_60Hz   =    {88, 144, 232, 1400,  1864,  3,  4,  32, 1050,  1089,   1,1,0,AR_16_9,121750};
...
//others
//code struct video_timing video_1200x1920_60Hz    ={180,60, 160, 1200, 1600,  35, 10, 25, 1920, 1990, 1, 1, 0, AR_16_9,191040}; 
//code struct video_timing video_1920x720_60Hz     ={88, 44, 148, 1920, 2200,  5,  5,  20, 720,  750,  1, 1, 0, AR_16_9,100000}; 
  ...
  u8 lanes;
	...
	if(lt9611.mipi_port_cnt==single_port_mipi)
	{
		...
		else//portB input
		{
			...
			printf("\r\nport B lane 0 settle = 0x%bx",HDMI_ReadI2C_Byte(0xA0));
			printf("\r\nport B lane 1 settle = 0x%bx",HDMI_ReadI2C_Byte(0xA2));
			printf("\r\nport B lane 2 settle = 0x%bx",HDMI_ReadI2C_Byte(0xA4));
			printf("\r\nport B lane 3 settle = 0x%bx",HDMI_ReadI2C_Byte(0xA6));
...
//	  //infoFrame_en = (AIF_PKT_EN|AVI_PKT_EN|SPD_PKT_EN);//9611 RX为MIPI，不需要发SPD_PKT
		  ...
		  do 
			{
					...
					for(i = 0; i < cec_msg->tx_data_buff[0]; i++)
							printf("0x%02bx, ",cec_msg->tx_data_buff[i + 1]);
...
//        return; 
    ...
    printf("0x%02bx, ",cec_msg->rx_data_buff[i + 1]);
...
void LT9611_Pcr_MK_Print(void)
{
	...
	for(loopx = 0; loopx < 10; loopx++)
	{
	  printf("\r\nPCRstable_bit4:0x%bx",HDMI_ReadI2C_Byte(0x97));
		...
		printf(" 0x%bx",HDMI_ReadI2C_Byte(0xb5));
		printf(" 0x%bx",HDMI_ReadI2C_Byte(0xb6));
		printf(" 0x%bx",HDMI_ReadI2C_Byte(0xb7));
		Timer0_Delay1ms(500);
	}
	#endif
}
